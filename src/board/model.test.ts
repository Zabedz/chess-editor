...
describe('BoardModel', () => {
  ...
  it('sets the en passant square on a double push an enemy pawn can take', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e2', { color: 'w', role: 'p' })
    model.put('d4', { color: 'b', role: 'p' })
    model.move('e2', 'e4')
    expect(model.getEnPassant()).toBe('e3')
    expect(model.fen()).toContain(' b - e3 ')
  })

  it('leaves the en passant square empty when no enemy pawn can take', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e2', { color: 'w', role: 'p' })
    model.move('e2', 'e4')
    expect(model.getEnPassant()).toBeNull()
  })

  it('captures en passant after a double push, removing the passed pawn', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e5', { color: 'w', role: 'p' })
    model.put('d7', { color: 'b', role: 'p' })
    model.setTurn('b')
    model.move('d7', 'd5')
    expect(model.getEnPassant()).toBe('d6')
    model.move('e5', 'd6')
    expect(model.get('d6')).toEqual({ color: 'w', role: 'p' })
    expect(model.get('d5')).toBeUndefined()
    expect(model.get('e5')).toBeUndefined()
    expect(model.getTurn()).toBe('b')
  })

  it('reports an en passant capture as a capture', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e5', { color: 'w', role: 'p' })
    model.put('d5', { color: 'b', role: 'p' })
    model.setPosition(model.getPieces(), 'w', 'd6')
    const listener = vi.fn()
    model.subscribe(listener)
    model.move('e5', 'd6')
    expect(listener).toHaveBeenCalledWith({ kind: 'move', capture: true })
  })

  it('clears the en passant square after the next move', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e2', { color: 'w', role: 'p' })
    model.put('d4', { color: 'b', role: 'p' })
    model.put('h7', { color: 'b', role: 'p' })
    model.move('e2', 'e4')
    expect(model.getEnPassant()).toBe('e3')
    model.move('h7', 'h6')
    expect(model.getEnPassant()).toBeNull()
  ...
  it('clears the en passant square on a piece edit', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e2', { color: 'w', role: 'p' })
    model.put('d4', { color: 'b', role: 'p' })
  ...
  it('undoes an en passant capture, restoring both pawns, the turn, and the target', () => {
    const model = new BoardModel()
    model.clear()
    model.put('e5', { color: 'w', role: 'p' })
    model.put('d7', { color: 'b', role: 'p' })
    ...
    model.move('e5', 'd6')
    model.undo()
    expect(model.get('e5')).toEqual({ color: 'w', role: 'p' })
    expect(model.get('d5')).toEqual({ color: 'b', role: 'p' })
    expect(model.get('d6')).toBeUndefined()
    expect(model.getTurn()).toBe('w')
    expect(model.getEnPassant()).toBe('d6')
  })

  it('does not treat a diagonal move from the wrong rank as en passant', () => {
    const model = new BoardModel()
    model.clear()
    model.put('c4', { color: 'w', role: 'p' })
    model.put('d4', { color: 'b', role: 'p' })
    model.setPosition(model.getPieces(), 'w', 'd6')
    model.move('c4', 'd6')
    expect(model.get('d6')).toEqual({ color: 'w', role: 'p' })
    expect(model.get('d4')).toEqual({ color: 'b', role: 'p' })
    expect(model.get('c4')).toBeUndefined()
  })

  it('takes the en passant square from a loaded position', () => {
    const model = new BoardModel()
    model.clear()
    model.setPosition(model.getPieces(), 'b', 'e6')
    expect(model.getEnPassant()).toBe('e6')
    expect(model.fen()).toContain(' e6 ')
  })
})
